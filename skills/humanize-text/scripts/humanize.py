#!/usr/bin/env python3
"""Humanize text with universal and profile-specific style guidance."""
...
from essay_context import build_humanize_prompt, load_essay, load_profile
from io_utils import EXIT_INVALID_INPUT, EXIT_RUNTIME_FAILURE, read_input_text, resolve_data_root


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Humanize text for EssayBuddy workflows")
...
def main() -> int:
    try:
        ...
        prompt = build_humanize_prompt(text=text, profile=profile)
        final_text = run_codex(prompt, timeout=args.timeout)
        ...
        sys.stderr.write(f"Humanize failed: {exc}\n")
